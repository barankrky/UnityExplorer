...
namespace UnityExplorer.MCP.Runtime
{
    ...
    public sealed class McpNativeProtocolHandler : IMcpRequestHandler
    {
        ...
        private static McpJsonValue BuildTools(McpGameExecutorOptions options)
        {
            ...
            AddTool(result, "search_objects", "Search game objects", "Search Unity objects and return stable handles for later calls. kind selects the coarse search type (gameobject, component, scriptableobject, asset) when type is omitted. path filters by hierarchy path substring; scene filters by scene name or handle; exact matches the name exactly; include_inactive=false hides objects inactive in the hierarchy; include_explorer=true also returns UnityExplorer's own UI objects.", Schema(Prop("query", String()), Prop("scene", String()), Prop("type", String()), Prop("kind", String()), Prop("path", String()), Prop("include_inactive", Boolean()), Prop("include_explorer", Boolean()), Prop("exact", Boolean()), Prop("limit", Integer(1, maxResults))), true, false, true);
